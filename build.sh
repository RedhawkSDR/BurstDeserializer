...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e BurstDeserializer.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/BurstDeserializer-1.0.0
        tar czf ${tmpdir}/BurstDeserializer-1.0.0.tar.gz --exclude=".svn" -C ${tmpdir} BurstDeserializer-1.0.0
        rpmbuild -ta ${tmpdir}/BurstDeserializer-1.0.0.tar.gz
        rm -rf $tmpdir
    else
